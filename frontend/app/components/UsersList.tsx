...
const UserStats: React.FC<UserStatsProps> = ({ users, classes }) => (
  <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mt-12">
    <div className={`${classes.cardBg} rounded-lg p-4 text-center border ${classes.borderColor}`}>
      <p className={classes.secondaryText}>Total Users</p>
      <p className={`${classes.textColor} text-2xl font-bold`}>{users.length}</p>
    </div>
    <div className={`${classes.cardBg} rounded-lg p-4 text-center border ${classes.borderColor}`}>
      <p className={classes.secondaryText}>Active</p>
      <p className={`${classes.textColor} text-2xl font-bold`}>{users.filter(u => u.status === 'active').length}</p>
    </div>
    <div className={`${classes.cardBg} rounded-lg p-4 text-center border ${classes.borderColor}`}>
      <p className={classes.secondaryText}>Inactive</p>
      <p className={`${classes.textColor} text-2xl font-bold`}>{users.filter(u => u.status === 'inactive').length}</p>
    </div>
    <div className={`${classes.cardBg} rounded-lg p-4 text-center border ${classes.borderColor}`}>
      <p className={classes.secondaryText}>Admins</p>
      <p className={`${classes.textColor} text-2xl font-bold`}>{users.filter(u => u.role === 'Admin').length}</p>
    </div>
  </div>
);
  ...
  return (
    <div className={`${classes.bgColor} min-h-screen p-6`}>
      <div className="max-w-6xl mx-auto">
        <div className="mb-8">
          <h1 className={`${classes.textColor} text-3xl font-bold mb-2`}>Users Management</h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredUsers.length > 0 ? (
            ...
            <div className={`${classes.textColor} col-span-full text-center py-12`}>
