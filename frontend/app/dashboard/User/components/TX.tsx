...
import {
  ...
  getPageById,
  MayanDocument,
  MayanDocumentDetails,
} from "../../apiService";
import Logo from "../../../../public/vercel.svg";
import { useRouter } from "next/navigation";
import { signOut } from "next-auth/react";
import { Button } from "@/components/animate-ui/components/buttons/button";
import { LogOut, Loader2, FileText, Calendar, Eye } from "lucide-react";
...
const extractFromUrl = (url: string) => {
  ...
  return {
    ...
    fileId: parts[parts.indexOf("files") + 1],
    pageId: parts[parts.indexOf("pages") + 1],
  };
};
...
const fetchDocuments = async () => {
  try {
    ...
    await Promise.all(
      docs.map(async (doc: MayanDocument) => {
        if (!doc.id) return;

        try {
          const docResponse = await getMayanDocumentById(doc.id.toString());
          const pages: MayanDocumentDetails = docResponse.data;

          if (pages.results?.length > 0) {
            const firstPage = pages.results[0];
            const { documentId, fileId, pageId } = extractFromUrl(firstPage.url);

            const imageResponse = await getPageById(documentId, fileId, pageId);
